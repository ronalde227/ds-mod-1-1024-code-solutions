...
def backward_stepwise_train(X_train, y_train, feature_cols):
    ...
    y_pred_test = model.predict(xval)
    y_pred_test_original = target_transformer.inverse_transform(y_pred_test.reshape(-1,1)).flatten()
    yval_original = target_transformer.inverse_transform(yval.reshape(-1,1)).flatten()

    rmse = mean_absolute_error(y_pred_test_original, yval_original)**0.5
    ...
    for i in range(1, k):
        ...
        for feat in remaining_features:
            ...
            y_pred_test = model.predict(xval[features])
            y_pred_test_original = target_transformer.inverse_transform(y_pred_test.reshape(-1,1)).flatten()
            yval_original = target_transformer.inverse_transform(yval.reshape(-1,1)).flatten()

            rmse = mean_squared_error(y_pred_test_original, yval_original)**0.5
